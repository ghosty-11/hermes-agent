...
class _RunLaunch:
    ...
    strict_runtime_request: Optional[Dict[str, Any]]
    strict_runtime: Optional[Dict[str, Any]] = None
...
async def _handle_runs(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    if _strict.is_strict_run_input(raw_input):
        ...
        lock_conflict = _strict.strict_session_lock_conflict(
            requested_model=strict["model"],
            requested_provider=strict["provider"],
            persisted_lock=_strict_persisted_lock(
                self, _strict, strict_effective_session
            ),
            session_model_override=self._session_model_override_for(
        ...
        if lock_conflict:
            return _json_error(
                _openai_error,
                lock_conflict,
                code="model_lock_conflict",
                status=409,
            )
        user_message = list(strict["text_parts"])
        if strict["has_image"]:
            user_message.append(
                {
                    "type": "image_url",
                    "image_url": {"url": strict["image_url"]},
                }
            )
        conversation_history, instructions, stored_session_id, history_err = (
            _resolve_conversation_history(
                self, body, raw_input, _openai_error=_openai_error
            )
        )
    ...
    agent_overrides = _api_server._request_agent_overrides(
    ...
    launch = _RunLaunch(
        ...
        agent_kwargs=dict(
            ...
            **{k: agent_overrides.get(k) for k in (
                "requested_model", "requested_provider", "model_options", "strict_controls"
            )}),
...
def _run_agent_sync(
    self, run: _RunLaunch, agent, approval_notify, publish_runtime, *, _api_server
):
    ...
    with self._profile_scope(run.request_profile):
        try:
            ...
            if run.strict is not None:
                from gateway.platforms import api_server_strict_runs as _strict

                try:
                    run.strict_runtime = _confirm_strict_run_before_inference(
                        self,
                        agent,
                        _strict,
                        strict=run.strict,
                        strict_runtime_request=run.strict_runtime_request,
                        session_id=session_id,
                    )
                except _strict.StrictRunError as exc:
                    r = {
                    ...
                    publish_runtime(run.strict_runtime)
...
async def _execute_run(self, run: _RunLaunch, *, _api_server) -> None:
    ...
    def _finish(status: str, extra: Optional[dict] = None, **fields: Any) -> None:
        """Publish terminal status and event with any confirmed strict runtime."""
        extra = dict(extra or {})
        if run.strict_runtime is not None:
            extra["runtime"] = run.strict_runtime
        if run_id in self._shutdown_interrupted_run_ids:
            status = "interrupted"
