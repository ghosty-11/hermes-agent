...
class InProcessCronScheduler(CronScheduler):
    ...
    def _start_multiplex(
        ...
        def tick_adapters_for(profile_name):
            # A secondary uses its own bot when connected. An empty map may
            # borrow the primary only under the explicit shared-bot topology;
            # otherwise only exact enabled profile routes may borrow it.
        ...
        while not stop_event.is_set():
            ...
            # Read from the primary home once per cycle, before profile scopes.
            # A runtime flip therefore takes effect on the next tick.
