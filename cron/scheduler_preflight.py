...
def _satellite_delivery_via_primary_enabled() -> bool:
    """Whether the primary gateway fronts every satellite's cron delivery."""
    try:
        from hermes_constants import get_default_hermes_root, get_hermes_home

        primary_home = Path(get_default_hermes_root()).expanduser()
        current_home = Path(get_hermes_home()).expanduser()
        if primary_home.resolve(strict=False) == current_home.resolve(strict=False):
            return False
        config_path = primary_home / "config.yaml"
        if not config_path.exists():
            return False
        from hermes_cli.config import read_user_config_raw
...
def _live_delivery_platform(platform_name: str, live_adapters) -> bool:
    """Return whether an in-process native or Relay transport can deliver."""
...
def _preflight_check_delivery(job: dict, live_adapters=None) -> Optional[str]:
    """Check delivery targets resolve to usable platforms.

    ``local``/``origin``/``all`` require no gateway config. Unknown platforms
    always block. A live native or Relay transport wins over profile-local
    credential discovery; otherwise an explicitly unconnected platform blocks.
    Configuration-load failures fail open.
    """
    ...
    for platform_name in platform_parts:
        ...
        # Multiplex: a satellite served by primary adapters reads unconnected.
        if (
            platform_name.lower() not in connected
            and not _delivery_platform_routed_from_primary_gateway(platform_name)
            and not _satellite_delivery_via_primary_enabled()
        ):
            return (
...
def _preflight_job_config(
    job: dict, cfg: dict, live_adapters=None
) -> Optional[str]:
