...
def _resolve_cron_disabled_toolsets(cfg: dict) -> list[str]:
    ...
    ``messaging`` and ``clarify`` are interactive-only. ``memory`` is always
    disabled because unattended jobs must not read or mutate the profile's
    persistent representation. ``cronjob`` is denied by default for loop
    prevention; ``cron.allow_agent_scheduling`` lifts only that denial.
    ``agent.disabled_toolsets`` is layered on top so a per-job allowlist cannot
    widen past the profile policy.
    """
...
def _preflight_or_block(
    job: dict, job_id: str, job_name: str, cfg: dict, *, live_adapters=None
) -> Optional[tuple]:
    ...
    try:
        if _cron_preflight_enabled(cfg):
            _pf_reason = _preflight_job_config(
                job, cfg, live_adapters=live_adapters
            )
...
def _resolve_cron_agent_setup(
    job: dict, job_id: str, job_name: str, jc, *, live_adapters=None
) -> _CronAgentSetup:
    ...
    setup.blocked = _preflight_or_block(
        job, job_id, job_name, _cfg, live_adapters=live_adapters
    )
...
def _construct_cron_agent(AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir, session_id, session_db):
    ...
    return AIAgent(
        ...
        # Unattended cron runs must not read or update persistent MEMORY.md/USER.md.
        skip_memory=True,
...
def run_job(
    ...
    execution_id: Optional[str] = None, live_adapters=None,
) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        _cfg = jc.cfg
        model = jc.model
        setup = _resolve_cron_agent_setup(
            job, job_id, job_name, jc, live_adapters=live_adapters
        )
        if setup.blocked is not None:
...
def _run_one_job_body(
    ...
    try:
        ...
        _run_kwargs["live_adapters"] = adapters
