...
class SharedRouteAdapters:
    ...
    ``allow_unrouted`` is the DOWNSTREAM composition of this class with
    ``gateway.satellite_cron_delivery_via_primary`` (see that knob below).
    Upstream's exact-route resolution is tried first and always wins; the
    knob only decides what an unmatched target does. Upstream fails it
    closed, which is right for a bot-per-profile deployment. On this host
    ONE bot fronts the whole multiplex and only one channel is a route, so
    failing closed silently re-blocks four enabled jobs whose targets are
    named channels and inbox threads nobody wants a satellite answering
    inbound on. With the knob on, an unmatched target borrows the primary
    adapter — the same bot that fronted it before a07370abd0.
    """

    def __init__(self, primary_adapters, routes, allow_unrouted=False) -> None:
        self._primary = dict(primary_adapters or {})
        self._routes = list(routes or [])
        self._allow_unrouted = bool(allow_unrouted)

    def __bool__(self) -> bool:
        return bool(self._primary) and (
            bool(self._routes) or self._allow_unrouted
        )
    ...
    def get(self, platform, target=None, default=None):
        ...
        for route in self._routes:
            ...
            if route.matches(str(route.platform), chat_id=chat_id, thread_id=thread_id):
                return adapter
        return adapter if self._allow_unrouted else default
...
def _run_one_job_body(
    ...
    *,
    ...
    try:
        ...
        try:
            # Downstream: live_adapters is our multiplex-delivery seam; pass it only
            # when the gateway supplied adapters so upstream run_job stubs (which do
            # not know the kwarg) keep working in tests and standalone CLI runs.
            _live_adapter_kwargs = {} if adapters is None else {"live_adapters": adapters}
