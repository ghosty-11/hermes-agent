...
def custom_provider_session_affinity_headers(
    ...
    The lookup is provider-aware as well as base_url-aware: a custom entry can share a base_url
    with another, so the entry name is what identifies whose ``session_affinity_header`` applies.
    """
    try:
        from hermes_cli.config import get_custom_provider_session_affinity_header

        header = get_custom_provider_session_affinity_header(base_url=base_url, provider=provider)
    except Exception:
        return {}
    if not header:
        return {}
...
def merge_session_affinity_headers(
    ...
    headers.update(custom_provider_session_affinity_headers(provider, base_url, session_id))
    if headers:
