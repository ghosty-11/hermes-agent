...
def _refuse_strict_tool_calls(
    agent, assistant_message, messages: list, effective_task_id: str
) -> bool:
    """Deny every dispatch entry when a strict run enforces no tools."""
    if getattr(agent, "_strict_no_tools", False) is not True:
        return False
    message = "Tool execution is not permitted for this run."
    ...
    for tool_call in list(getattr(assistant_message, "tool_calls", None) or []):
        ...
        messages.append(
            make_tool_result_message(
                name, result, call_id, effect_disposition="none"
            )
        )
        _ToolCallRef(name, {}, effective_task_id, call_id, []).emit_post(
            agent,
            result,
