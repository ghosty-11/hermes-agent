...
class ClientLifecycleMixin:
    ...
    def _request_slot(self, slot_attr: str) -> dict:
        cache = getattr(self, slot_attr, None)  # lazy: tests build agents via AIAgent.__new__
        if cache is None:
            setattr(self, slot_attr, cache := {})
            _reset_slot(cache)
        cache.setdefault("created_at", None)
        cache.setdefault("serial", 0)
        cache.setdefault("last_call_reused", None)
        return cache
        ...
        with self._openai_client_lock():
            ...
            if cache["client"] is None:
                cache.update(
                    ...
                    serial=int(cache.get("serial") or 0) + 1,
                    last_call_reused=False,
                )
    ...
    def request_client_provenance(self, client: Any) -> dict[str, Any]:
        """Describe the cached request client without claiming TCP reuse."""
        info = {
            "client_id": None,
            "transport_id": None,
            "client_serial": None,
        ...
        try:
            ...
            info["transport_id"] = id(transport) if transport is not None else None
