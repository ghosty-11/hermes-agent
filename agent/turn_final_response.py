...
def finish_text_response(
    ...
    if getattr(agent, "_hermes_strict_run", False) is True:
        from agent.strict_run import terminal_failure
        from agent.trajectory import has_incomplete_scratchpad

        if (
            has_incomplete_scratchpad(final_response)
            or finish_reason in {"incomplete", "tool_calls"}
            or not agent._strip_think_blocks(final_response).strip()
        ):
            return _verdict(
                "return",
                terminal_failure(
    ...
    # Ordered after the strict-run gate on purpose: that gate judges what the model
    # actually produced, so a reasoning-only turn still reads as an incomplete answer
    # there, while the promotion decides only what this call returns.
    ...
    final_response = _promoted or final_response
