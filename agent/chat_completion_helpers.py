...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # Downstream diagnostic kept across the 2026-09-02 merge: upstream's
        # top-of-loop extraction records `usage_obj` but no longer writes the
        # `_diag` usage summary the stream-stall reports read. Summarize once
        # here — after the loop, so no `continue` path can skip it.
