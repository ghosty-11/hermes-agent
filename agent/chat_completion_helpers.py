...
def _with_stream_emitters(agent, run, *, stream_diag=None):
    """Bracket one provider streaming attempt with observer hooks."""
    ...
    try:
        ...
        if end is not None:
            snapshot = stream_diag() if callable(stream_diag) else None
    ...
    if end is not None:
        snapshot = stream_diag() if callable(stream_diag) else None
            ...
            final_text=_stream_final_text(response),
            finished=True,
            error=None,
            stream_diag=snapshot,
        )
...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _new_diag(self) -> dict:
        ...
        diag.update({
            "est_context_tokens": estimate_request_context_tokens(self.api_kwargs),
            "tools": len(self.api_kwargs.get("tools") or []),
            "stale_threshold": self._stream_stale_timeout,
            "thread": threading.current_thread().name,
            "attempt": getattr(self, "_stream_attempt_number", None),
            "max_attempts": getattr(self, "_stream_max_attempts", None),
            "client": None,
        })
    ...
    def _diag_snapshot(self):
        snapshot = _stream_diag_snapshot(self.clients.diag)
        if snapshot is None:
            return None
        for field in (
            "est_context_tokens", "tools", "stale_threshold", "thread",
            "attempt", "max_attempts", "client",
        ):
            snapshot[field] = self.clients.diag.get(field)
        snapshot["base_url"] = self.agent.base_url or ""
        try:
            next_rung = int(getattr(self.agent, "_fallback_index", 0) or 0)
            snapshot["fallback_rung"] = max(0, next_rung - 1) if next_rung else 0
            snapshot["fallback_active"] = next_rung > 0
        except Exception:
            snapshot["fallback_rung"] = snapshot["fallback_active"] = None
        return snapshot

    def _count_chunk(self, diag, chunk) -> None:
        """Stamp liveness for a real provider chunk; diagnostics are best-effort."""
        self.last_chunk_time["t"] = time.time()
        self.agent._touch_activity("receiving stream response")
        with contextlib.suppress(Exception):
            _stream_diag_note_chunk(diag, self.last_chunk_time["t"])
    ...
    def _open_chat_stream(self, stream_kwargs: dict[str, Any]):
        ...
        with contextlib.suppress(Exception):
            self.clients.diag["client"] = self.agent.request_client_provenance(
                request_client
            )
        self.last_chunk_time["t"] = time.time()
        ...
        for chunk in _iter_provider_stream_chunks(stream, response=lambda: self._attempt_stream_response):
            ...
            if not chunk.choices:
                ...
                if usage is not None:
                    _diag["usage"] = _usage_summary(usage)
                continue
    ...
    def _call(self):
        ...
        try:
            while _stream_attempt < _max_stream_retries + self._compat_retries:
                _stream_attempt += 1
                self._stream_attempt_number = _stream_attempt + 1
                self._stream_max_attempts = _max_stream_retries + 1
                ...
                try:
                    self.result["response"] = _with_stream_emitters(
                        self.agent,
                        lambda: self._call_wire(stream_attempt_id),
                        stream_diag=self._diag_snapshot,
                    )
                    return  # success
                except Exception as e:
                    self._close_managed_stream()
