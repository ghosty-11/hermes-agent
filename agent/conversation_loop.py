...
class _LoopState:
    ...
    strict_request_attempted: bool = False
    strict_tool_followup: bool = False
...
def _run_api_retry_loop(agent, s: _LoopState) -> Optional[Dict[str, Any]]:
    ...
    while s.retry_count < s.max_retries:
        if getattr(agent, "_hermes_strict_run", False) is True:
            if s.strict_request_attempted and not s.strict_tool_followup:
            ...
            s.strict_request_attempted = True
            s.strict_tool_followup = False
...
def _run_conversation_turn(
    ...
    while (s.api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        from agent.strict_run import apply_transport_controls

        apply_transport_controls(agent)
        ...
        try:
            ...
            if _v.action == "continue":
                if (
                    getattr(agent, "_hermes_strict_run", False) is True
                    and s.assistant_message.tool_calls
                ):
                    s.strict_tool_followup = True
                continue
