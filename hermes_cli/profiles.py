...
# Usage counters and lock ownership belong to the profile that produced them.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    def _ignore(directory: str, names: List[str]) -> set:
        ...
        ignored.update(entry for entry in names if entry in _SKILL_USAGE_SIDECARS)
        if at_root:
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    if source_skills.is_dir():
        # Upstream's junction-preserving copytree (a live special file would abort a plain
        # copytree) with the house skill-usage sidecars excluded on top of
        # _non_exportable_entries.
        _copytree_keep_junctions(
            source_skills,
            profile_dir / "skills",
            lambda directory, names: _non_exportable_entries(directory, names)
            | {entry for entry in names if entry in _SKILL_USAGE_SIDECARS},
            dirs_exist_ok=True,
        )
