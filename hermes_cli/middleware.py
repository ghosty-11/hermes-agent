...
def _apply_request_chain(
    ...
    """Feed the effective payload through callbacks in registration order."""
    from hermes_cli.plugins import get_plugin_manager

    current = kwargs[payload_key]
    original_key = "original_request" if payload_key == "request" else "original_args"
    callbacks = list(get_plugin_manager()._middleware.get(kind, []))
    for callback in callbacks:
        call_kwargs = middleware_payload(**kwargs)
        call_kwargs[payload_key] = current
        call_kwargs[original_key] = _safe_copy(original)
        try:
...
def apply_tool_request_middleware(
    tool_name: str, args: Dict[str, Any], *, original_args: Dict[str, Any] | None = None,
    **context: Any
) -> RequestMiddlewareResult:
    ...
    return _apply_request_chain(
        TOOL_REQUEST_MIDDLEWARE, "args", trace, original_snapshot,
        tool_name=tool_name, args=current_args, original_args=original_snapshot, **context,
    )
